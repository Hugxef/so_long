...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/25 13:10:36 by hugrene           #+#    #+#             */
/*   Updated: 2022/05/25 13:10:37 by hugrene          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_perso_up(t_data *data, int key)
{
	if (ft_is_move_allow(data, key))
	{
		...
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, \
		data->perso.img.ptr_img, data->perso.pos_x, data->perso.pos_y);
	}
	ft_check_case(data);
}
...
int	deal_key(int key, t_data *data)
{
	...
	if (data->is_end == 0)
	{
		...
		ft_printf("%d/%d collectable	", data->count_collect, \
